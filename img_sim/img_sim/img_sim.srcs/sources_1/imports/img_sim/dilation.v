...
// wire    [7:0]   row1_data_r;     //??shift_ram????????��??? ?????????
// wire    [7:0]   row2_data_r;

// wire    [7:0]   row1_data;      //3?��???
// wire    [7:0]   row2_data;
// wire    [7:0]   row3_data;
...
//     end
//     else if(vld[0]) begin                   //??in_data????????????????????��???0??????��??--> in_data == out_data
...
// //????????????
// assign  row1_data = row1_data_r;         //????��??��??shift_ram1?????
// assign  row2_data = row2_data_r;         //????��??��???shift_ram0?????
